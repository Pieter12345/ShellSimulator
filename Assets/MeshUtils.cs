	...
	public static List<int>[] getSortedVertexTriangles(int[] triangles, int numVertices) {
		...
		for(int vertexInd = 0; vertexInd < sortedVertexTriangles.Length; vertexInd++) {
			...
			if(triangleList.Count > 0) {
				...
				// Insert initial triangle.
				sortedTriangleList.Add(triangleList[0]);
				triangleList.RemoveAt(0);

				// Move all triangles from the triangle list to the sorted triangle list.
				while(triangleList.Count > 0) {
					bool changed = false;

					// Insert a triangle that is adjacent to an already added triangle if available.
					...
					// Select a random triangle to insert if no adjacent match was found.
					// This means that there are multiple triangle groups for one vertex.
					if(!changed) {
						sortedTriangleList.Add(triangleList[0]);
						triangleList.RemoveAt(0);
					}
