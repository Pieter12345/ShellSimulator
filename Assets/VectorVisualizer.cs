...
public class VectorVisualizer {
	...
	public void visualize(Vec3D[] positions, VecD vectors, double scale = 1d) {
		this.visualize(new VecD(positions), vectors, scale);
	}

	public void visualize(VecD positions, VecD vectors, double scale = 1d) {
		float vecThicknessScale = 0.02f;
		for(int i = 0; i < positions.length / 3; i++) {
			Vector3 origin = new Vector3((float) positions[3 * i], (float) positions[3 * i + 1], (float) positions[3 * i + 2]) + offset;
			Vector3 vec = new Vector3((float) vectors[3 * i], (float) vectors[3 * i + 1], (float) vectors[3 * i + 2]) * (float) scale;
			...
			if(vecMag != 0f) {
				...
				gameObj.transform.localScale = new Vector3(vecThicknessScale, 0.5f * vecMag, vecThicknessScale);
